...
class GsQuat
 { public :
   ...
   public :
    ...
    GsQuat ( const GsMat& m, char fmt='C' ) { set(m,fmt); }
...
/*! Converts the rotation in the given rotation matrix to quaternion format;
    fmt can be 'C' or 'L' for column (default) or line major format. */
void mat2quat ( const GsMat& m, GsQuat& q, char fmt='C' );
...
void quat2mat ( const GsQuat& q, 
                ...
                float& x3, float& y3, float& z3, char fmt='C' );
...
void quat2mat ( const GsQuat& q, GsMat& m, char fmt='C' );
