...
void AppWindow::glutKeyboard ( unsigned char key, int x, int y )
 {
   switch ( key )
    { case ' ': _viewaxis = !_viewaxis; redraw(); break;
	  ...
	  case 'q': _capsule.changed = true; nfaces += nfacesChange; redraw(); break;
	  case 'a': _capsule.changed = true; nfaces -= nfacesChange; redraw(); break;

		  //shifts the light along the z axis
	  case 'w': _capsule.changed = true; lightz += 0.2f; redraw(); break;
	  case 's': _capsule.changed = true; lightz -= 0.2f; redraw(); break;

		  //turns normals on and off
	  case 'v': _capsule.changed = true; _viewnormal = false; redraw(); break; //off
	  case 'c': _capsule.changed = true; _viewnormal = true; redraw(); break; //on

		  //flat and smooth shading
	  case 'z': _capsule.changed = true; shadType = true; redraw(); break; //smooth
	  case 'x':_capsule.changed = true; shadType = false; redraw(); break; //flat																			case 'd': _smoothcapsule.changed = true; rb -= rbChange; redraw(); break;*/
	}
 }
...
void AppWindow::glutDisplay ()
 {
   ...
   if(_capsule.changed)
   {
	   _capsule.build(len,rt,rb,nfaces,shadType);
   ...
   if (_viewcapsule) _capsule.draw(stransf, sproj, light, material);
