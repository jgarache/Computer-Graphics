...
void AppWindow::glutKeyboard ( unsigned char key, int x, int y )
 {
   switch ( key )
    { case ' ': _viewaxis = !_viewaxis; break;
	  ...
	  case '0': drawCurve = false; break; //no curves
	  case '1': typeCurve = 1; drawCurve = true;   break; //lagrange
	  case '2': typeCurve = 2; drawCurve = true;   break; //bezier

	  case '3': typeCurve = 3; drawCurve = true;   break; //bspline
	  case '4': typeCurve = 4; drawCurve = true;   break; //Catmull-Rom spline
	  case '5': typeCurve = 5; drawCurve = true;   break; //Bessel-Overhauser spline

	  case 'z': threeDSwitch = true; _curve.changed = true; break; //3D surface option
	  case 'x': _curve.changed = !_curve.changed; break; //updates
...
void AppWindow::glutDisplay ()
 {
   ...
   if(_curve.changed)_curve.build(_poly, segments, threeDSwitch,typeCurve);
   ...
   if (drawCurve) _curve.draw(_str, _spr, sun);
