...
void SoPoly::curveEval(float segs, bool mode, GsArray<GsVec>& actCurve)
{
	if (mode) {
		for (int i = 0; i < V.size() - 1; i++) {
			actCurve.push(V[i]);
			for (int j = 1; j < segs + 1; j++) {
				float x = V[i].x + (((V[i + 1].x - V[i].x) / segs)*j);
				actCurve.push(eval_lagrange(x, V));
			}
		}
		
		if (V.size() != 0) {
			actCurve.push(V[V.size()-1]);
		}


		for (int i = 0; i < actCurve.size(); i++) {
			std::cout << actCurve[i].x << " : " << actCurve[i].y << "==";
		}
		std::cout << std::endl;
	}
	else{
		eval_bezier(segs, actCurve, V);
